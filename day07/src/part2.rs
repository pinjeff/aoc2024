...
fn eval(lhs: usize, rhs: usize, op: Operator) -> usize {
    match op {
        ...
        Operator::Concat => lhs * 10_usize.pow(((rhs as f64).log10()) as u32 + 1) + rhs,
    }
}

fn generate_combinations(length: usize) -> Vec<Vec<Operator>> {
    if length == 0 {
        return vec![vec![]]; // Base case: return a single empty combination
    }
    ...
    for combo in smaller_combinations {
        ...
        result.push([combo.clone(), vec![Operator::Concat]].concat());
    }
